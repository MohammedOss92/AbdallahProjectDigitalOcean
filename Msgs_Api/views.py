...
def msgtypes_api_show(request):
    # ������� ������ ���� ����� ��� new_msgs_text ����� 0
    data = MeesageType.objects.exclude(messages__new_msgs_text=0).values(
...
class SnippetsListViewWhereidtypeidpa(ListAPIView):
    ...
    def get_queryset(self):
        # استبعاد الصفوف حيث new_msgs_text=1
        queryset = Messages.objects.exclude(new_msgs_text=1)

        # إذا تم توفير ID_Type_id في الطلب، استخدمه للبحث
...
=======
            queryset = queryset.filter(ID_Type_id=id_type_id)
>>>>>>> 4d07779287d1b468c273939f4173e00c13a1ddae

        # ارجاع النتيجة مرتبة تنازلياً حسب id
        return queryset.order_by('-id')

    def list(self, request, *args, **kwargs):
        queryset = self.get_queryset()

        # قم بالحصول على صفحة البيانات باستخدام التقسيم
